...
from attacks.refined_score_attack_pq import RefinedScoreAttackPQRank
...
class RankLeakageExperiment(Experiment):
    ...
    def run_experiment_increased_known_document_size(self,):
        ...
        base_location = self.dirname+"/results/different_rank_information/"
        ...
        for partial_value in partial_values:
            ...
            while correct_experiment_count < self.experiment_count and error_count < self.error_count:
                ...
                attacker = RefinedScoreAttackPQRank(1)
    ...
    def run_experiment_similar_dataset(self, ):
        ...
        save_name_basic = base_location + f"known_data_similar_basic"
        save_name_refined = base_location + f"known_data_similar_refined"

        save_name_basic_dict = base_location + f"/mappings/known_data_similar_basic_mappings"
        save_name_refined_dict = base_location + f"/mappings/known_data_similar_refined_mappings"
        ...
        while correct_experiment_count < self.experiment_count:
            ...
            chosen_vocab = distribution_creator.get_queries_uniform(
                queries, int(self.fraction_observed * len(queries)))
            known_queries_id = np.random.choice(len(chosen_vocab), nr_known_queries, replace=False)
            known_queries = [chosen_vocab[x] for x in known_queries_id]

            attacker = ScoreAttackPQRank()

            try:
                mappings_basic, mappings_documents_basic = attacker.execute_attack(
                    self.documents, chosen_vocab, known_queries, self.domain, {}, self.similar_documents)
                basic_success = True

            except PQTreeException:
                print("error occurred")

            attacker = RefinedScoreAttackPQRank(18, True)
            ...
            if basic_success and refined_success:
                correct_experiment_count += 1
                self.write_to_file_own_attacks(
                    save_name_basic, mappings_basic, mappings_documents_basic, self.domain, nr_known_queries, len(chosen_vocab), self.density)
    ...
    def run_experiment(self):
        ...
        self.run_experiment_similar_dataset()
